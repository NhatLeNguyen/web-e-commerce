...
public class GoogleLoginHandler(ILogger<GoogleLoginHandler> logger) : IRequestHandler<GoogleLoginCommand, Result<LoginResponse>>
{
    public Task<Result<LoginResponse>> Handle(GoogleLoginCommand request, CancellationToken cancellationToken)
    {
        ...
        logger.LogWarning("Google login not yet configured. Provide GOOGLE_CLIENT_ID in appsettings.");
